...
// Find plants belonging to a specific user by user ID
...
async function add(plant, userId) {
  ...
  const addedPlant = await findById(id);
  return { id, ...addedPlant };
}

// Add a new plant associated with a user to the database
function addPlantToUser(newPlant, userId) {
  return db('plants').insert({ ...newPlant, user_id: userId });
}

// Find a plant by its ID in the database
function findById(id) {
  return db('plants').where({ id }).first();
}

// Update a plant's information in the database by its ID
...
// async function add(plant, userId) {
//   // Assign the user_id to the plant object
//   plant.user_id = userId;

//   // Insert the plant into the database and retrieve its ID
//   const [id] = await db('plants').insert(plant);

//   // Retrieve the details of the added plant by its ID
//   const addedPlant = await findById(id);

//   // Return an object containing the ID and details of the added plant
//   return { id, ...addedPlant };
// }
